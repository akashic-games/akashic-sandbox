...
targetVersions.forEach(function(version) {
	...
	var outputPackageJson = JSON.parse(fs.readFileSync(path.join(jsDirPath, version, "versions.json"), "utf8"));
	["dependencies", "devDependencies", "optionalDependencies"].forEach(function(item) {
		...
		for (var libName in inputPackageJson[item]) {
			...
			if (inputPackageJson[item][libName] !== outputPackageJson[libName]) {
				console.log(
					"version of "
					+ libName
					+ " expect "
