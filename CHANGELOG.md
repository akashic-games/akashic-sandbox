...
## UnreleasedChanges
* エンティティの範囲外が描画されている時エラーを投げる処理を以下のように変更
  * surfaceがHTMLCanvasのもののみ範囲外描画されているかチェックする
  * エラーを投げるのではなく警告を出すだけの処理に変更
* 上記処理を行うかどうかのフラグを意味する`warn.drawOutOfCanvas`というパラメータをsandboxConfigに追加
